...
# Known support of epsilon_jm
eps_lower <- -300
eps_upper <- 300

# Storage for market-level bounds
lower_bounds <- c()
upper_bounds <- c()

# Store detailed market-level bounds
bounds_detail <- data.frame(
  market = integer(),
  profile = character(),
  LB1 = numeric(),
  LB2 = numeric(),
...
for (m in 1:nrow(data)) {
  ...
  piM1 <- data$piM1_bar[m]
  piM2 <- data$piM2_bar[m]
  piD1 <- data$piD1_bar[m]
  piD2 <- data$piD2_bar[m]
  ...
  if (a1_m == 0 && a2_m == 0) {
    ...
    lower_bounds <- c(lower_bounds, LB1, LB2)
  ...
  if (a1_m == 1 && a2_m == 0) {
    UB1 <- piM1 - eps_lower   # = piM1 + 300
    LB2 <- piD2 - eps_upper   # = piD2 - 300
    
    upper_bounds <- c(upper_bounds, UB1)
    lower_bounds <- c(lower_bounds, LB2)
  ...
  # Case 3: (0,1)
  if (a1_m == 0 && a2_m == 1) {
    UB2 <- piM2 - eps_lower   # = piM2 + 300
    LB1 <- piD1 - eps_upper   # = piD1 - 300
    
    upper_bounds <- c(upper_bounds, UB2)
    lower_bounds <- c(lower_bounds, LB1)
  ...
  if (a1_m == 1 && a2_m == 1) {
    UB1 <- piD1 - eps_lower   # = piD1 + 300
    UB2 <- piD2 - eps_lower   # = piD2 + 300
    
    upper_bounds <- c(upper_bounds, UB1, UB2)
...
# sample set
FC_lower <- max(lower_bounds, na.rm = TRUE)
FC_upper <- min(upper_bounds, na.rm = TRUE)
